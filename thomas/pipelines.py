...
class ThomasPipeline:

    # Database setup
    conn = sqlite3.connect('thomas.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `thomas`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM thomas WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO thomas (date, title, link, content)"
